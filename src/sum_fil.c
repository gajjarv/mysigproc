...
main(int argc, char *argv[]) 
{
	...
	FILE *output;
    char buffer[13313];
    double spectra_sum[13313];  //max channels 4096
  ...
  if (output_nbits == 8 && input_nbits == 8) {
		 ...
		 /* bump past header for all input files */
		  ...
		  min = 0;
                  ...
                  fbuffer8 = (unsigned char *) malloc (input_nchans * sizeof(char));
                  fspectra_sum8 = (unsigned char *) malloc (output_nchans * sizeof(char));
		  fspectra_sum = (unsigned char *) malloc (output_nchans * sizeof(float));
                  ...
                  for (j = 0; j < qlen; j++){
                     ...
                     memset(fspectra_sum8, 0x0, output_nchans * sizeof(char));

                         for(i = 0; i < tcollapse; i++) {
                                   fread(fbuffer8, sizeof(char), (input_nifs * input_nchans), fileptr[0]);
				   /*
				   for(k = 0; k < input_nchans; k++){	 					   
				   	printf("%d %c\n",k,fbuffer8[k]);		
				   }
				   */		
                                   for(k = 0; k < input_nchans; k = k + fcollapse) {
                                                for (m = 0; m < fcollapse; m++) {
							fspectra_sum8[k/fcollapse] = (fspectra_sum8[k/fcollapse] + fbuffer8[k + m]);
						}
                                   }
                         }
                         ...
                         for(k=0; k < output_nchans; k++) {
                                 //printf("%f\n", );
                                 if(fspectra_sum8[k] > max) max = fspectra_sum8[k];
                                 if((fspectra_sum8[k] < min && fspectra_sum8[k] > 0.0) || min == 0.0) min = fspectra_sum8[k];
                         }
		...
		for (j = 0; j < qlen; j++){
                     ...
                     memset(fspectra_sum8, 0x0, output_nchans * sizeof(char));
                         ...
                         for(k=0; k < output_nchans; k++) {
                                 if(fspectra_sum8[k] > 0.0) {
                                        gsl_histogram_increment(spectra_quant, fspectra_sum8[k]);
                                        nelements++;
                                }
                         }
		...
		/* outer loop, read through all spectra, quantize to 8 bits, write to file */
		 ...
		 for (j = 0; j < ((input_datasize) / (long long) (input_nifs * input_nchans)); j++){
			 ...
			 for(i = 0; i < tcollapse; i++) {
				   ...
				   for(k = 0; k < nchans; k = k + fcollapse) {   		
				   		for (m = 0; m < fcollapse; m++) fspectra_sum8[k/fcollapse] = (fspectra_sum8[k/fcollapse] + fbuffer8[k + m]/(fcollapse*tcollapse));
				   }
				   
			 }					
			 
			 //fwrite(&fspectra_sum8,sizeof(char),output_nchans,output);

			  	 
			 for(k=0; k < output_nchans; k++) {		
				  quantval = quantize(fspectra_sum8[k], quantmin, quantmax);
				  fwrite(&quantval, sizeof(char), 1, output);	 				 
				  //fspectra_sum8[k] = fspectra_sum8[k]/fcollapse; 
				  //fwrite(&fspectra_sum8[k], sizeof(char), 1, output);	 				 
			 }
		...
		/*
			 ...
			 for(i = 0; i<nfiles; i++) {
				  ...
				  for(k=0; k<nchans; k++) spectra_sum[k] = (spectra_sum[k] + ((double) buffer[k]));
			  }	
			  //for(i=0; i<128; i++) printf("%f\n", spectra_sum[i]);
			  for(i=0; i<nchans; i++) {
				gsl_histogram_increment(spectra_quant, spectra_sum[i]);
			  }
			  //usleep(5000000);	
		  }	
		...
		*/
		...
		//  for (j = 0; j < (output_datasize / (long long) (output_nifs * output_nchans)); j++){
			 /* read n spectra (1 spectra x n files), sum */
			/*
			 for(i=0;i<output_nchans;i++) spectra_sum[i] = 0.0;
			 for(i = 0; i<nfiles; i++) {
				  fread(buffer, sizeof(char), (output_nifs * output_nchans), fileptr[i]);
				  for(k=0; k<128; k++) spectra_sum[k] = (spectra_sum[k] + ((double) buffer[k]));
			  }
