...
void FLIP(FILE* file, FILE *outfile, double flipTime,double flipFreq, double nohdr);
...
void print_usage(){
	...
	printf("--nohdr, -H	      : Do not change header value while reverse ordering\n");
	printf("\nOther options:\n");
...
int main (int argc, char** argv){
	...
	long_opt[long_opt_idx].name = "nohdr";
        long_opt[long_opt_idx].has_arg = no_argument;
        long_opt[long_opt_idx].flag = NULL;
        long_opt[long_opt_idx++].val = 'H';
	...
	if(flipTime==1 || flipFreq==1) FLIP(file,outfile,flipTime,flipFreq,nohdr); 	
	return 0;
}
...
void FLIP(FILE *file, FILE *outfile,double flipTime,double flipFreq, double nohdr){
	...
	if(flipTime==1) sprintf(newbuf,"nT_outfile.fil");
        if(flipFreq==1) sprintf(newbuf,"nF_outfile.fil");
        if(flipTime == 1 && flipFreq == 1) sprintf(newbuf,"nTnF_outfile.fil");

	outfile=fopen(newbuf,"w");
	...
	if(nohdr==1) sprintf(newbuf,"%s",source_name);
